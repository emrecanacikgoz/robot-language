...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        assert config.model_gpt["n_embd"] % config.model_gpt["n_head"] == 0,  f"Cannot parallelize since embedding size [{config.model_gpt['n_embd']}] is not divisable by number of heads [{config.model_gpt['n_head']}]" 
        
        # key, query, value projections for all heads, but in a batch
        self.c_attn = nn.Linear(config.model_gpt["n_embd"], 3 * config.model_gpt["n_embd"], bias=config.model_gpt["bias"])
        # output projection
        self.c_proj = nn.Linear(config.model_gpt["n_embd"], config.model_gpt["n_embd"], bias=config.model_gpt["bias"])
        ...
        if not self.flash:
            ...
            self.register_buffer("bias", torch.tril(torch.ones(config.model_gpt["block_size"], config.model_gpt["block_size"])).view(1, 1, config.model_gpt["block_size"], config.model_gpt["block_size"]))
...
class FeedForward(nn.Module):
    def __init__(self, config):
        ...
        if config.model_gpt["feedforward"] == "fc": 
            self.c_fc    = nn.Linear(config.model_gpt["n_embd"], 4*config.model_gpt["n_embd"], bias=config.model_gpt["bias"])
            self.c_proj  = nn.Linear(4*config.model_gpt["n_embd"], config.model_gpt["n_embd"], bias=config.model_gpt["bias"])
        elif config.model_gpt["feedforward"] == "cnn": 
            self.c_fc    = Conv1D(config.model_gpt["n_embd"], 4*config.model_gpt["n_embd"])
            self.c_proj  = Conv1D(4*config.model_gpt["n_embd"], config.model_gpt["n_embd"])
        else:
